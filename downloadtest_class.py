...
class download():
    ...
    def getFile(self, url, title, group_name):
        ...
        os.chdir('/Users/jeffgjf/PycharmProjects/SeleniumTest/')
        try:
            if os.path.exists(group_name):
                os.chdir(os.path.join('/Users/jeffgjf/PycharmProjects/SeleniumTest/', group_name))
            else:
                os.mkdir(group_name)
                os.chdir(os.path.join('/Users/jeffgjf/PycharmProjects/SeleniumTest/', group_name))
        except Exception as e:
    ...
    def getFile2(self, url, title, group_name):
        ...
        try:
            if os.path.exists(title + '.pdf'):
                print '%s has been downloaded, Do not need to download agian!' %title
            else:
    ...
    def toDownload(self, cop_url):
        ...
        # 下载文件并保存
        self.getFile2(url, title, group_name)


        print '%s has been downloaded from %s. %s' %(title, cop_url, ctime())
    ...
    def run(self, start_page, end_page):
        ...
        for i in range(start_page, end_page):
            ...
            count_page += 1
            print 'All amount of cops in this web: %s by now.' %all_count_cops
            print '##############################################################################################################################################'
